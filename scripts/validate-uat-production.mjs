...
check('Version v4.x', /^4\./.test(String(pkg.version)))
...
check('Versioned database migration runner', exists('scripts/db-migrate.mjs') && !!pkg.scripts['db:migrate:deploy'])
check('Production start applies migrations automatically', pkg.scripts.prestart === 'node scripts/db-migrate.mjs')
check('Build validates DB schema contract', pkg.scripts.prebuild?.includes('validate-db-schema-contract.mjs'))
check('Neon direct migration URL documented', read('docs/RAILWAY-DEPLOYMENT.md').includes('DATABASE_MIGRATION_URL'))
check('Railway migration guide', exists('docs/RAILWAY-DEPLOYMENT.md') && read('docs/RAILWAY-DEPLOYMENT.md').includes('Pre-Deploy Command'))
check('Production DB schema push opt-in', payload.includes("push: process.env.PAYLOAD_DB_PUSH === 'true'"))
...
check('UAT checklist', exists('docs/QA-HANDOFF.md'))
check('Deployment guide', exists('docs/DEPLOYMENT.md'))
