...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    const [item, defaults] = await Promise.all([getItem((await params).slug), getDefaultContentMedia()])
    if (!item) return {}
    const image = mediaUrl(item.seoImage || item.cover) || defaults.recruitment
    return { title: item.seoTitle || item.title, description: item.seoDescription || `Thông tin tuyển dụng ${item.title}`, openGraph: { title: item.seoTitle || item.title, description: item.seoDescription || `Thông tin tuyển dụng ${item.title}`, images: image ? [image] : [] } }
  } catch { return {} }
}
