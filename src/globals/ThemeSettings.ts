...
const colorField = (name: string, label: string, defaultValue?: string, opts?: Record<string, unknown>): Field => ({
  ...
  admin: {
    components: { Field: '/src/components/admin/ColorPickerField#default' },
...
export const ThemeSettings: GlobalConfig = {
  ...
  fields: [
    ...
    // =========================================================================
    // CẤU HÌNH MẪU BANNER ĐẦU TRANG DÙNG CHUNG (PAGE HERO BANNER)
    // =========================================================================
    {
      name: 'pageHero',
      label: 'Mẫu Banner đầu trang dùng chung (Page Hero)',
      type: 'group',
      admin: {
        description: 'Cấu hình đồng bộ màu nền, màu chữ, cỡ chữ và khoảng cách cho Banner đầu trang của tất cả các trang: Bác sĩ, Tin tức, Thông báo, Đấu thầu, Bảng giá, Văn bản, Chuyên khoa, Lịch khám, v.v.',
      },
      fields: [
        {
          name: 'bgType',
          ...
          options: [
            { label: 'Dải màu Gradient y tế (Khuyến nghị)', value: 'gradient' },
            { label: 'Một màu trơn (Solid color)', value: 'solid' },
          ],
          admin: { width: '50%' },
        },
        {
          name: 'paddingVertical',
          label: 'Khoảng đệm trên dưới (px)',
          type: 'number',
          defaultValue: 22,
          min: 12,
          max: 60,
          admin: { width: '50%', description: 'Mặc định chuẩn y tế nhỏ gọn: 22px' },
        },
        {
          type: 'row',
          admin: {
            condition: (_data: any, siblingData: any) => siblingData?.bgType !== 'solid',
          },
          fields: [
            colorField('bgGradientStart', 'Màu bắt đầu Gradient', '#072b4c'),
            colorField('bgGradientMiddle', 'Màu giữa Gradient', '#0754a8'),
            colorField('bgGradientEnd', 'Màu kết thúc Gradient', '#0878d1'),
          ],
        },
        colorField('bgSolidColor', 'Màu nền trơn (khi chọn Solid)', '#0754a8', {
          admin: {
            condition: (_data: any, siblingData: any) => siblingData?.bgType === 'solid',
          },
        }),
        {
          type: 'row',
          fields: [
            colorField('titleColor', 'Màu chữ tiêu đề', '#ffffff'),
            {
              name: 'titleFontSize',
        ...
        },
        {
          type: 'row',
          fields: [
            colorField('descColor', 'Màu chữ mô tả', '#e2f1fc'),
            {
              name: 'descFontSize',
        ...
        },
        {
          type: 'row',
          fields: [
            colorField('breadcrumbColor', 'Màu chữ thanh điều hướng (Breadcrumb)', '#bae6fd'),
            colorField('breadcrumbLinkColor', 'Màu đường link Trang chủ', '#e0f2fe'),
          ],
        },
      ],
    },
    ...
    // =========================================================================
    // CẤU HÌNH MẪU KHỐI NỘI DUNG DÙNG CHUNG (SECTION GLOBAL SETTINGS)
    // =========================================================================
    {
      name: 'sectionGlobal',
      label: 'Mẫu Khối nội dung dùng chung (Sections)',
      type: 'group',
      admin: {
        description: 'Cấu hình đồng bộ màu nền, màu chữ, cỡ chữ và khoảng đệm cho toàn bộ các Section nội dung: Kỹ thuật chuyên sâu, Đội ngũ chuyên gia, Chuyên khoa, Tin tức nổi bật, Thông báo, Đấu thầu, Lịch khám, v.v.',
      },
      fields: [
        {
          type: 'row',
          fields: [
            colorField('bgColor', 'Màu nền khối Section', '#ffffff'),
            {
              name: 'borderRadius',
              label: 'Bo góc khối (px)',
              type: 'number',
              defaultValue: 18,
              min: 0,
              max: 40,
        ...
        },
        {
          ...
          fields: [
            ...
            {
              name: 'contentWidth',
              label: 'Chiều rộng khung tối đa (px)',
              type: 'number',
              defaultValue: 1180,
              min: 960,
              max: 1400,
        ...
        },
        {
          type: 'row',
          fields: [
            colorField('eyebrowColor', 'Màu nhãn Eyebrow', '#0878d1'),
            {
              name: 'eyebrowSize',
              label: 'Cỡ chữ nhãn Eyebrow (px)',
              type: 'number',
              defaultValue: 11,
              min: 9,
              max: 18,
        ...
        },
        {
          type: 'row',
          fields: [
            colorField('titleColor', 'Màu chữ tiêu đề Section', '#124064'),
            {
              name: 'titleSize',
              label: 'Cỡ chữ tiêu đề Section (px)',
              type: 'number',
              defaultValue: 26,
              min: 18,
              max: 40,
        ...
        },
        {
          type: 'row',
          fields: [
            colorField('descColor', 'Màu chữ mô tả Section', '#657f92'),
            {
              name: 'descSize',
              label: 'Cỡ chữ mô tả Section (px)',
              type: 'number',
              defaultValue: 13,
              min: 11,
              max: 20,
        ...
        {
          name: 'headingGap',
          label: 'Khoảng cách giữa tiêu đề và nội dung (px)',
          type: 'number',
          defaultValue: 18,
          min: 8,
          max: 48,
