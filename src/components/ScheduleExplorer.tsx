...
type Props = {
  ...
  medpro: string
  tabOrder?: string[]
  tabs?: ScheduleTabConfig[]
  compact?: boolean
}
type ScheduleKind = 'daily' | 'nurse' | 'weekly' | 'attachments' | 'emergency'
...
function ScheduleCard({ item, kind, featured = false }: { item: any; kind: ScheduleKind; featured?: boolean }) {
  const label = kind === 'emergency'
    ? 'LỊCH TRỰC CẤP CỨU'
    : kind === 'nurse'
      ? `LỊCH ĐIỀU DƯỠNG · ${formatDate(item.date)}`
      : kind === 'daily'
  ...
  const summary = kind === 'emergency'
    ...
    : kind === 'nurse'
      ? (item.summary || item.note || 'Lịch phân công Điều dưỡng - Nữ hộ sinh công tác tại các Khoa / Phòng.')
      : kind === 'daily'
  ...
  const detail = kind === 'emergency'
    ...
    : (kind === 'daily' || kind === 'nurse')
      ? (item.startTime && item.endTime ? `${item.startTime} – ${item.endTime} · ${item.room || 'Phòng khám cập nhật tại quầy'}` : (item.date ? `Ngày ${formatDate(item.date)}` : ''))
      : `Áp dụng: ${formatDate(item.weekStart || item.validFrom)} ${(item.weekEnd || item.validTo) ? `– ${formatDate(item.weekEnd || item.validTo)}` : ''}`
  const placeholder = kind === 'emergency' ? 'TRỰC' : kind === 'nurse' ? 'ĐD' : kind === 'daily' ? (item.date ? new Date(item.date).toLocaleDateString('vi-VN', { day: '2-digit' }) : 'LỊCH') : kind === 'weekly' ? '7' : (item.fileFormat || 'TỆP')
...
export function ScheduleExplorer({
  ...
  attachments,
  emergency = [],
  tabOrder = ['emergency', 'nurse', 'daily', 'weekly', 'attachments'],
  tabs: configuredTabs,
  compact = false,
}: Props) {
  const builtInMeta: Record<string, { label: string; items: any[]; kind: ScheduleKind; empty: string }> = {
    ...
    nurse: { label: 'Lịch điều dưỡng', items: nurse, kind: 'nurse', empty: 'Chưa có lịch điều dưỡng.' },
  ...
  const tabDefinitions = useMemo(() => {
    ...
    if (valid.length) {
      rawTabs = valid.map((item, index) => {
        ...
        return {
          ...
          label: item.label?.trim() || builtIn?.label || `Lịch khám ${index + 1}`,
          kind: builtIn?.kind || 'attachments' as ScheduleKind,
          items: [...(builtIn?.items || []), ...manualItems],
          empty: builtIn?.empty || 'Nội dung đang được cập nhật.',
        }
      })
    } else {
      const keys = tabOrder.length ? tabOrder : ['emergency', 'nurse', 'daily', 'weekly', 'attachments']
    ...
    // Ẩn hoàn toàn các tab không có nội dung (items.length === 0) theo yêu cầu người dùng
    return rawTabs.filter((t) => t.items.length > 0)
  }, [attachments, configuredTabs, daily, emergency, nurse, tabOrder, weekly])
  ...
  useEffect(() => {
    if (!tabDefinitions.some((item) => item.key === tab)) {
      setTab(tabDefinitions[0]?.key || '')
    }
  }, [tab, tabDefinitions])

  const activeDefinition = tabDefinitions.find((item) => item.key === tab) || tabDefinitions[0]

  // Lấy danh sách ngày cho tab hiện tại (nếu là daily hoặc nurse)
  const dates = useMemo(() => {
    if (!activeDefinition || (activeDefinition.kind !== 'daily' && activeDefinition.kind !== 'nurse')) return []
    return Array.from(new Set(activeDefinition.items.map((item: any) => item.date).filter(Boolean)))
  }, [activeDefinition])

  const renderPanel = (definition: typeof tabDefinitions[number]) => {
    const isDateFilterable = definition.kind === 'daily' || definition.kind === 'nurse'
    const items = isDateFilterable && selectedDate !== 'all'
      ? definition.items.filter((item: any) => !item.date || item.date === selectedDate)
      : definition.items
    const expanded = !compact && expandedTab === definition.key
    const visibleItems = expanded ? items : items.slice(0, 5)

    return (
      <div className="scheduleTabPanel">
        {items.length === 0 ? (
          <div className="empty-state">{definition.empty}</div>
        ) : (
          <>
            ...
            {!compact && items.length > 5 && (
              <div className="tabViewAllWrap">
                <button className="tabViewAllButton" type="button" onClick={() => setExpandedTab(expanded ? null : definition.key)}>
                  {expanded ? 'Thu gọn danh sách ↑' : `Xem tất cả ${items.length} nội dung →`}
                </button>
              </div>
            )}
          </>
  ...
  const showDateFilter = (activeDefinition?.kind === 'daily' || activeDefinition?.kind === 'nurse') && dates.length > 1

  return (
    <div className={`scheduleExplorer ${compact ? 'compactScheduleExplorer' : ''}`}>
      {/* Thanh header kết hợp: Danh sách Tabs bên trái & Bộ lọc ngày ngang hàng bên phải */}
      <div className="scheduleExplorerToolbar">
          ...
          <div className="scheduleTabs" role="tablist">
            {tabDefinitions.map((item) => (
              <button
                ...
                onClick={() => { setTab(item.key); setExpandedTab(null); setSelectedDate('all') }}
              >
        ...
        {showDateFilter && (
          <div className="scheduleDateFilter">
            <label htmlFor="scheduleDateSelect">Chọn ngày xem lịch:</label>
            <select
              id="scheduleDateSelect"
              value={selectedDate}
              onChange={(event) => { setSelectedDate(event.target.value); setExpandedTab(null) }}
            >
              <option value="all">Tất cả các ngày</option>
              {dates.map((date) => (
                <option value={date as string} key={date as string}>
