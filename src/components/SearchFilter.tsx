...
export function SearchFilter({ items, kind = 'news', initialCategory = 'all' }: { items: any[]; kind?: string; initialCategory?: string }) {
  ...
  const label = kind === 'notice' ? 'thông báo' : kind === 'procurement' ? 'nội dung mua sắm' : kind === 'recruitment' ? 'tin tuyển dụng' : kind === 'schedule' ? 'lịch phân công' : kind === 'vaccination' ? 'nội dung tiêm chủng' : kind === 'document' ? 'văn bản' : kind === 'science' ? 'hoạt động khoa học' : 'tin bài'
    ...
    <section className="contentDirectory" aria-label={`Danh sách ${label}`}>
      ...
      {categories.length > 0 && (
        <div className="categoryDirectory" aria-label="Chuyên mục">
          ...
          {categories.map(c => {
            ...
            const isEmerg = c === 'Lịch trực cấp cứu'
            return (
              <button
                type="button"
                key={String(c)}
      ...
      <div className="contentDirectoryHeading">
        <div>
          ...
          <h2>{category === 'all' ? (kind === 'news' ? 'Tin mới cập nhật' : kind === 'notice' ? 'Thông báo mới cập nhật' : kind === 'schedule' ? 'Lịch khám & trực mới cập nhật' : kind === 'vaccination' ? 'Thông tin tiêm chủng mới cập nhật' : kind === 'document' ? 'Văn bản mới cập nhật' : kind === 'science' ? 'Hoạt động khoa học mới cập nhật' : 'Nội dung mới cập nhật') : category}</h2>
        </div>
      ...
      {filtered.length === 0 ? <div className="contentDirectoryEmpty"><strong>Không tìm thấy kết quả phù hợp.</strong><span>Thử từ khóa khác hoặc chọn lại chuyên mục.</span></div> : (
        <div className={`contentCardGrid ${styles.contentCardGrid}`}>
          {filtered.map((item, idx) => {
            const isEmerg = norm(item.category || item.type) === 'Lịch trực cấp cứu'
            const imgFit = item.coverFit === 'fill' ? 'fill' : (item.coverFit === 'contain' ? 'contain' : 'cover')
            const imgPos = item.coverFit === 'cover-top' || item.coverPosition === 'top'
              ? 'top center'
              : (item.coverFit === 'cover-bottom' || item.coverPosition === 'bottom'
                ? 'bottom center'
                : 'center center')
            return (
              <a className={`contentDirectoryPostCard ${styles.contentDirectoryPostCard} ${isEmerg ? styles.cardEmergency : ''}`} href={item.href || '#'} key={item.id || idx} target={item.external ? '_blank' : undefined} rel={item.external ? 'noopener noreferrer' : undefined}>
                <div
                  className={`postCardMediaWrap ${styles.postCardMediaWrap}`}
                  style={item.coverFit === 'contain' ? { background: '#f4f8fb' } : undefined}
                >
                  {item.coverUrl ? (
                    <img
                    ...
                    <div className={`postCardPlaceholder ${styles.postCardPlaceholder} ${isEmerg ? styles.placeholderEmergency : ''}`}>
                      <span>{isEmerg ? 'TRỰC CẤP CỨU' : kind === 'notice' ? 'THÔNG BÁO' : kind === 'procurement' ? 'MUA SẮM' : kind === 'recruitment' ? 'TUYỂN DỤNG' : kind === 'science' ? 'KHOA HỌC' : 'TIN BÀI'}</span>
                    </div>
                  )}
                  {(item.category || item.type) && (
                    <span className={`postCardBadge ${styles.postCardBadge} ${isEmerg ? styles.postCardBadgeEmergency : ''}`}>
                      {isEmerg && (
                ...
                <div className={`postCardBody ${styles.postCardBody}`}>
                  <div className={`postCardDate ${styles.postCardDate} ${isEmerg ? styles.postCardDateEmergency : ''}`}>
                  ...
                  <div className={`postCardFooter ${styles.postCardFooter}`}>
                    <span className={`postCardAction ${styles.postCardAction} ${isEmerg ? styles.actionEmergency : ''}`}>Xem chi tiết <i aria-hidden="true">→</i></span>
                  </div>
                </div>
              </a>
            )
