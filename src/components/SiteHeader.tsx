...
export async function SiteHeader() {
  ...
  const items = nav?.items?.filter((x: any) => x.visible !== false) || [
    ...
    },
    {
      ...
      children: [
        ...
        { label: 'Phác đồ điều trị', url: '/phac-do-dieu-tri' },
      ],
    },
    {
      label: 'Dành cho người bệnh',
      url: '/danh-cho-nguoi-benh',
      children: [
        ...
        { label: 'Tra cứu phản ánh', url: '/gop-y/tra-cuu' },
    ...
    },
    {
      ...
      url: '/bang-gia',
      children: [
        { label: 'Bảng giá viện phí & BHYT', url: '/bang-gia' },
        { label: 'Tiêm chủng vắc xin', url: '/tiem-chung' },
      ],
    },
    {
      label: 'Tin tức & Công khai',
      url: '/tin-tuc',
      children: [
        { label: 'Tin tức & Sự kiện', url: '/tin-tuc' },
        { label: 'Thông báo bệnh viện', url: '/thong-bao' },
        { label: 'Đấu thầu – Mua sắm y tế', url: '/dau-thau-mua-sam' },
        { label: 'Văn bản – Tài liệu y tế', url: '/van-ban' },
        { label: 'Phác đồ điều trị chuẩn', url: '/phac-do-dieu-tri' },
      ],
    },
  ...
  const COLOR_SCHEME_PRESETS: Record<string, { bg: string; titleColor: string; subtitleColor: string; sloganColor: string; overlay: string }> = {
    'navy-gold': {
      bg: '#0a1f3c',
      titleColor: '#f8d568',
      subtitleColor: '#e8c84a',
      sloganColor: '#fbd44e',
      overlay: 'rgba(10, 31, 60, 0.06)',
    },
    'green-white': {
      bg: '#f0faf5',
      titleColor: '#0a6b3a',
      subtitleColor: '#0a6b3a',
      sloganColor: '#15803d',
  ...
  const colorScheme = brandAppearance?.colorScheme || 'custom'
  const schemePreset = colorScheme !== 'custom' ? (COLOR_SCHEME_PRESETS[colorScheme] || null) : null

  // Apply preset overrides if not 'custom'
  const resolvedBg = schemePreset ? schemePreset.bg : (brandAppearance?.headerBackgroundColor || '#ffffff')
  const resolvedTitleColor = schemePreset ? schemePreset.titleColor : (brandAppearance?.titleColor || '#0756b4')
  const resolvedSubtitleColor = schemePreset ? schemePreset.subtitleColor : (brandAppearance?.subtitleColor || '#0878D1')
  const resolvedSloganColor = schemePreset ? schemePreset.sloganColor : (brandAppearance?.sloganColor || brandAppearance?.subtitleColor || '#0a9b50')
  const resolvedOverlay = schemePreset ? schemePreset.overlay : (brandAppearance?.backgroundOverlay || 'rgba(255,255,255,0.88)')

  // Name font
  const nameFontFamily = brandAppearance?.nameFontFamily && brandAppearance.nameFontFamily !== 'inherit' ? brandAppearance.nameFontFamily : undefined
  const nameFontWeight = brandAppearance?.nameFontWeight || '800'
  const nameTextEffect = brandAppearance?.nameTextEffect || 'none'

  // Slogan styling
  const sloganFontWeight = brandAppearance?.sloganFontWeight || '500'
  const sloganItalic = brandAppearance?.sloganItalic === true
  const sloganTextEffect = brandAppearance?.sloganTextEffect || 'none'

  const backgroundImage = mediaUrl(brandAppearance?.backgroundImage)
  const mastheadStyle = {
    '--masthead-background': resolvedBg,
    ...
    '--masthead-background-overlay': resolvedOverlay,
    '--masthead-min-height': `${brandAppearance?.minHeight || 110}px`,
    '--masthead-title-color': resolvedTitleColor,
    '--masthead-title-size': `${brandAppearance?.titleFontSize || 17}px`,
    '--masthead-subtitle-color': resolvedSubtitleColor,
    '--masthead-slogan-color': resolvedSloganColor,
    ...
    // Name typography
    '--masthead-name-font': nameFontFamily,
    '--masthead-name-weight': nameFontWeight,
    // Slogan typography
    '--masthead-slogan-weight': sloganFontWeight,
  } as CSSProperties
        ...
        <div className="container hospitalMastheadInner">
          <a className="mastheadBrand" href="/" aria-label="Về trang chủ">
            ...
            <span className="mastheadBrandText">
              {brandAppearance?.showHospitalName !== false && (
                <strong
                  className={[
                    'mastheadHospitalName',
                    nameTextEffect !== 'none' ? `nameEffect-${nameTextEffect}` : '',
                  ].filter(Boolean).join(' ')}
                  style={{
              ...
              {settings?.headerShowSlogan !== false && (
                <small
                  className={[
                    'mastheadSlogan',
                    sloganTextEffect !== 'none' ? `sloganEffect-${sloganTextEffect}` : '',
                    sloganItalic ? 'sloganItalic' : '',
                  ].filter(Boolean).join(' ')}
                  style={{
                ...
                >
                  {sloganTextEffect === 'star-wrap' ? `★ ${slogan} ★` : slogan}
                </small>
              )}
            </span>
          </a>
