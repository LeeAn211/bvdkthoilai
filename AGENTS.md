...
This version has breaking changes — APIs, conventions, and file structure may all differ from your training data. Read the relevant guide in `node_modules/next/dist/docs/` only when Next.js behavior requires it. Heed deprecation notices.

This block may be re-added by `next dev`; keep it intact.
